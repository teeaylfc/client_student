...
class EditExam extends Component {
    ...
    getExamDefine() {
        dataService.getExamDefine(this.props.location.state._id)
            .then(res => {
                ...
                this.setState({
                    ...
                    nameExam: res.nameExam,
                    chooseSubject: subject,
                    chooseClass: res.nameClass.length > 0 ? {
                        classId: res.nameClass[0]
                    } : null,
                    questionLevel: res.level,
                    chooseTypeTime: {
                        value: res.typeTime,
                        label: res.typeTime + " phút"
                    },
                    checkPrivateExam: res.isPrivateExam,
                    selectedStartDate: res.isPrivateExam ? res.startTime : res.effectStartTime,
                    selectedEndDate: res.isPrivateExam ? null : res.effectExpiredTime,
                    choosePrivateExam: {
                        value: res.isPrivateExam,
                        label: res.isPrivateExam ? "Riêng tư" : "Công khai",
                    },

                    chooseGrade: JSON.parse(res.degreeNumber)
                }, () => this.getListClass())
    ...
    getQuestionPack() {
        ...
        var params = {
            ...
            typeTime: this.state.chooseTypeTime.value
        }
    ...
    validatorStep1() {
        ...
        if (!chooseGrade) return "Chưa chọn khối lớp"
        if (!chooseClass) return "Chưa chọn lớp học"
        if (!questionLevel) return "Chưa chọn mức độ đề"
        if (!chooseTypeTime) return "Chưa chọn loại hình thời gian"
        // if (!this.chooseIsOnline) return "Chưa chọn hình thức thi"
        if (!choosePrivateExam) return "Chưa chọn loại đề thi"
            ...
            <div className="flex flex-col w-full items-center">
                ...
                <div className="form-step-1">
                    ...
                    <div className="flex flex-row mb-24 items-center">
                        ...
                        {
                            chooseSubject && listSubject.length > 0 &&
                            <DropMenu
                                value={"subjectName"}
                                width="130px"
                                list={listSubject}
                                currentItem={chooseSubject}
                        ...
                        {
                            listGrade.length > 0 && chooseGrade && <DropMenu
                                width="100px"
                                list={listGrade}
                                currentItem={chooseGrade}
                        ...
                        <h4 className="text-white ml-40 mr-16">Lớp học</h4>
                        {
                            ...
                            <DropMenu
                                value="classId"
                                width="100px"
                                idParent={chooseGrade ? chooseGrade : null}
                                disabled={(chooseGrade && listClass.length > 0) ? false : true}
                                list={listClass}
                                currentItem={chooseClass}
                    ...
                    <div className="flex flex-row items-center">
                        ...
                        {
                            choosePrivateExam && isPrivateExam.length > 0 &&
                            <DropMenu
                                value="label"
                                width="130px"
                                currentItem={choosePrivateExam}
                                list={isPrivateExam}
                        ...
                        {
                            typeTime.length > 0 && chooseTypeTime &&
                            <DropMenu
                                value={"label"}
                                width="120px"
                                list={typeTime}
                                currentItem={chooseTypeTime}
    ...
    clickCreateExam = () => {
        ...
        var params = {
            ...
            startTime: this.state.selectedStartDate.toISOString(),
            expiredTime: this.choosePrivateExam.value ? null : this.state.selectedEndDate.toISOString(),
